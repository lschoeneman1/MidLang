...
/**
 * Main entry point for the MidLang Stage 2 interpreter.
 * 
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: midlang_stage2 <source_file.mid>" << std::endl;
        std::cout << "Example: midlang_stage2 examples/program.mid" << std::endl;
        return 1;
    }
        ...
        for (const auto& token : tokens) {
            if (token.type != TokenType::EOF_TOKEN) {
                ...
                switch (token.type) {
                    ...
                    case TokenType::PRINTLN: typeStr = "PRINTLN"; break;
                    case TokenType::INPUT_INT: typeStr = "INPUT_INT"; break;
                    case TokenType::INPUT_STRING: typeStr = "INPUT_STRING"; break;
